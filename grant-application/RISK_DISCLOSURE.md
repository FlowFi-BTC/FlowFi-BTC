...
| The sBTC token referenced is the real one | **True on mainnet, permanently.** `escrow.sbtc-contract` defaults to `SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token`. The `set-sbtc-contract` function used to repoint this for testnet development is **removed from the contract source entirely before mainnet deployment** (Milestone 2) — not merely left uncalled. There is no post-deploy override path in the mainnet build. |
...
**Applied to pilot business selection:** the primary pilot business (Open Hive Innovations Ltd) was selected because, as a technology-focused company, it is *reasoned to have* better practical access to sBTC than a typical fiat-revenue business — reducing, though not eliminating, mechanical-default risk. **To be precise about the strength of this assumption: as of this application, there is no formal evidence that Open Hive currently holds sBTC or routinely transacts in digital assets.** This is a sector-based judgment call made in the absence of confirmed evidence, not a verified fact, and it is disclosed as such.

Given that, our current estimate is that the probability of the pilot resolving as a **mechanical default** (fiat proceeds received, but sBTC not acquired/transferred in time) rather than a **credit default** (the underlying debtor simply not paying) is approximately **20–30%**. This estimate will be revisited and stated plainly, either way, in `PILOT_RESULT.md`.
...
The business and its capital provider may voluntarily agree, off-chain, to continue settlement efforts past the on-chain due date if fiat proceeds have already arrived but conversion hasn't completed. This would be an arrangement between the two parties and would not change what the contract itself enforces; any such arrangement will be disclosed in full in `PILOT_RESULT.md` if it occurs.
...
**Liveness risk:** If the admin does not call `release-funds` after a receivable is funded, the escrowed sBTC sits in the contract indefinitely — the business never receives the advance, and the funder cannot recover the funds themselves, since there is no time-lock or automatic release/refund path in v1.0.0. Likewise, if the admin does not call `mark-default` after a receivable passes its due date without repayment, the receivable simply remains recorded as FUNDED on-chain indefinitely — no automatic transition occurs. For this single-pilot, single-operator deployment, the admin is the same person coordinating the transaction in real time with direct contact with both parties, which eliminates the incentive misalignment that makes this dangerous at scale. A multisig admin or time-lock mechanism is a stated prerequisite before any expansion beyond this pilot.
...
- A full 46-test suite (26 registry + 20 escrow, including 2 full integration paths) covering authorization, funding, repayment, and default paths, self-reviewed against a documented security checklist, expanding to 70+ tests during Milestone 1 hardening
...
**Business side:** the primary pilot business is **Open Hive Innovations Ltd** (RC-9590869), a technology company selected because it is reasoned to have better practical access to sBTC than a fiat-revenue business (see Section 3 for the honest caveat on this assumption). **Uncle Tee's Schools** (RC-1917924) is retained as a documented, CAC-verifiable fallback — see [PILOT_READINESS.md](./PILOT_READINESS.md) for the full outreach record.

**Capital-provider side:** outreach is underway, primarily direct-message outreach to individuals identified as potential sBTC/BTC holders or ecosystem participants in the Stacks and sBTC community on X. As of this application: **5 people approached, 3 responded, 0 committed to participating.** Conversations to date remain exploratory, focused on explaining the pilot structure and the responsibilities and risks of participating. No provider has been secured — see [PILOT_READINESS.md](./PILOT_READINESS.md) for the current status.
...
been removed entirely. Milestone 3 has a single success criterion: completion of an end-to-end financing
cycle involving a real business and an **independent third-party sBTC capital provider** — one with no
affiliation with the project team, no shared wallet ownership, and no financial relationship with the
project that would make the transaction effectively self-funded. If no such provider is secured, Milestone
3 will be reported as incomplete in `PILOT_RESULT.md`, naming the absence of an independent provider as
the specific unmet condition. Team-controlled wallets, founder funds, associated parties, or related
entities will not be used to simulate or replace the role of an external provider — doing so would not
test the actual hypothesis this grant exists to prove, that an independent sBTC holder is willing to
finance a real-world receivable through this workflow.

**If a genuine third-party cycle is completed,** `PILOT_RESULT.md` will disclose, as the specific evidence distinguishing it from a self-funded or related-party transaction:
- The participating provider's public Stacks wallet address
- The approximate date and channel through which initial contact was established
- An explicit statement that the provider was not affiliated with the project team
- Evidence that the provider chose to participate independently
...
controlled by the lead developer (Oyewale Prudence, @ProdevappOFFICIAL). `CONTRACT-OWNER` is set to
`tx-sender` at deploy time. There is no multisig. This is disclosed as a centralization
risk, not presented as a feature. A multisig admin configuration is a named prerequisite
before any deployment beyond this single pilot. See Section 4 for what happens, concretely, if this
single admin does not call `release-funds` or `mark-default` in time.
