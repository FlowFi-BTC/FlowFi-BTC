...
| Document | Purpose |
|---|---|
| [APPLICATION_NARRATIVE.md](./APPLICATION_NARRATIVE.md) | Main application — problem statement, solution, progress, vision |
...
| **Key Technology** | Clarity, sBTC (SIP-010), Stacks Connect, two-contract architecture (`flowfi-escrow.clar` + `flowfi-registry.clar`) |
...
3. **Clean separation of concerns.** `flowfi-registry.clar` owns identity, verification, and receivable state. `flowfi-escrow.clar` owns sBTC custody, funding, and settlement. Neither contract does the other's job — see [TECHNICAL_ARCHITECTURE.md](./TECHNICAL_ARCHITECTURE.md).
