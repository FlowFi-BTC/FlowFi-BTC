...
| Item | Detail |
|---|---|
| Mainnet deployment | `flowfi-registry` and `flowfi-escrow` deployed to Stacks mainnet (registry first), wired via `set-escrow-contract`; `sbtc-contract` left at real-sBTC default `SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token` — never a mock, and `set-sbtc-contract` locked or heavily gated post-deploy (see `TECHNICAL_ARCHITECTURE.md`) |
...
| Named real capital provider | One real sBTC holder funds the receivable with a real, on-chain sBTC transaction via `fund-receivable` (admin then calls `release-funds`). Capital-provider outreach is underway but not yet resolved as of this application — see [PILOT_READINESS.md](./PILOT_READINESS.md) and `RISK_DISCLOSURE.md` §8. **Fallback:** If no external provider is secured by Week 10, the fallback plan activates (extended outreach, smaller ticket, or internal sBTC funding) to execute the complete on-chain lifecycle without stalling milestone delivery. |
